...
import asyncio, threading, time, json, queue, os, sys, math, subprocess, importlib
...
For screenshot analysis: infer from OCR text and state uncertainty if needed.
"""
...
# ---------- TTS (NON-BLOCKING, DOUBLE-PRESS SAFE, VOICE CYCLE) ----------
class TTS:
    """
    Uses a dedicated worker thread.
    No deadlocks: stop/voice-change never blocks the UI.
    """
    ...
    def stop(self):
        # instant, does not wait for locks held by speaking
    ...
    def cycle_voice(self) -> str:
        ...
        self._q.put(("SET_VOICE", getattr(v, "id", "")))
...
class SpeechIn:
    ...
    def listen_once(self, seconds=LISTEN_SECONDS):
...
class OrbUI:
    def __init__(self):
        ...
        self.mode = "idle"   # idle | listen | speak
        self.last_text = ""
        self._t0 = time.time()
    ...
    def tick(self, mic_level: float, speaking: bool):
        ...
        self.base = max(45.0, min_dim * 0.10)
        self.r = max(self.r, self.base)
...
class App:
    ...
    def handle_double_press(self):
        # STOP speaking — now safe (non-blocking)
        self.tts.stop()
        ...
        def run():
            while self.convo_mode and self.ui.running:
                ...
                try:
                    ...
                    text = self.stt.listen_once(LISTEN_SECONDS)
