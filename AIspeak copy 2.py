...
# Implements EVERYTHING requested in your last message:
#
# TTS:
# ✅ STOP does not deadlock / never disables future speech
# ✅ Immediate stop via _speak_id invalidation + engine.stop()
# ✅ speak_async() always re-enables speech
# ✅ safe voice switching (cancels current speech)
# ✅ reduced chunk size (max_len=120) for lower latency + faster interrupts
# ✅ stronger tts_format(): unicode normalize, emoji strip, symbol cleanup
# ✅ optional symbol_to_onomatopoeia()
#
# Performance:
# ✅ background gradient cached (huge FPS gain)
# ✅ orb physics decoupled from FPS: dt passed in
# ✅ ZCR computed/filtered only when needed
# ✅ adaptive FPS cap (think mode lower)
#
# Turn-taking (target behavior):
# ✅ AI never STARTS speaking while user is speaking
# ✅ If user speaks < 2s while AI speaking: AI continues (soft interjection recorded later)
# ✅ If user speaks >= 2s while AI speaking: AI stops (hard interruption), then listens
# ✅ user speech during AI speech is added to context AFTER AI finishes/stops
# ✅ AI answers only after user finishes + silence gap
# ================================================

import asyncio, threading, time, json, queue, sys, math, subprocess, importlib, heapq, re, unicodedata
...
class SoftAssociativeMemory:
    ...
    def build_context_hint(self, history: List[Dict[str, str]], max_items: int = 6) -> str:
        if not self.G:
            return ""
        return ("Long-term memory (soft priors):\n"
                "- Prefer concise answers unless asked otherwise.\n"
                "- If user interrupts you, shorten next response.\n")

# =============================================================
# TTS formatting (extended)
# =============================================================
...
def tts_format(text: str) -> str:
    ...
    text = re.sub(r'\(([^)]+)\)', r' pause \1 resume ', text)
...
# =============================================================
# TTS (dead-state-proof + latency-friendly)
# =============================================================
...
class TTS:
    ...
    def cycle_voice(self) -> str:
        ...
        with self._lock:
            ...
            v = self._voices[self._voice_idx]
        ...
        return getattr(v, "name", getattr(v, "id", "Unknown voice"))
            ...
            try:
                if cmd == "STOP":
                    ...
                    try:
                        ...
                        try:
                            self._engine.setProperty("rate", max(80, TTS_RATE - 20))
                        except Exception:
                            pass
...
class SpeechIn:
    ...
    def listen_until_silence(
        ...
        while total < max_total_sec:
            ...
            start_threshold = max(VOL_FLOOR, noise_rms * 1.9) if adapt_floor else VOL_FLOOR
...
class LLM:
    ...
    def reply(self, user_text: str) -> str:
        ...
        if mem_hint.strip():
            msgs.append({"role": "system", "content": mem_hint})
...
# =============================================================
# Orb UI (cached background + dt physics)
# =============================================================
...
class OrbUI:
    ...
    def set_mode(self, m: str):
        self.mode = m
        if m == "listen":
            self.core_color = (0, 255, 120)
            self.glow_color = (0, 180, 90)
            self.spring_hz = 5.0
            self.damping = 0.78
        elif m == "think":
            self.core_color = (255, 210, 80)
            self.glow_color = (200, 140, 40)
            self.spring_hz = 2.8
            self.damping = 0.86
        elif m == "speak":
            self.core_color = (80, 160, 255)
    ...
    def _get_bg(self):
        size = (self.w, self.h)
        if self._bg_cache is not None and self._bg_size == size:
            return self._bg_cache

        bg = pygame.Surface(size)
        for y in range(self.h):
            c = int(10 + 10 * math.sin((y / max(1, self.h)) * math.pi))
            pygame.draw.line(bg, (6, 8, 14 + c), (0, y), (self.w, y))

        self._bg_cache = bg
        self._bg_size = size
        return bg

    # dt-decoupled derivative spring (port style)
    def update_radius(self, mic_level, dt):
        dt = max(1e-4, float(dt))

        active = mic_level >= VOL_FLOOR
        if active:
            self.quiet_since = time.time()

        idle = (time.time() - self.quiet_since) >= IDLE_AFTER
        if idle:
            self.r = float(self.base)
            self.v = 0.0
            return

        combined = mic_level if active else 0.0
        r_t = self.base + combined * (min(self.w, self.h) * 0.45 - self.base)
        r_t = float(max(self.base, min(r_t, min(self.w, self.h) * 0.45)))

        w  = 1.5 * math.pi * self.spring_hz
        z  = self.damping * 10.0
        err = r_t - self.r

        a = -z * w * self.v + (w * w) * err
        self.v += (0.00085 * a + 0.7 * err * 0.05) * dt
        self.r += self.v * math.sqrt(abs(self.v + 0.001)) * dt

        max_r = min(self.w, self.h) * 0.45
        self.r = float(max(self.base, min(self.r, max_r))) / 1.05
        self.v *= 0.99
    ...
    def tick(self, mic_level: float, speaking: bool):
        now = time.time()
        dt = min(0.05, now - self._last_phys)
        self._last_phys = now
        self.update_radius(mic_level, dt)
        ...
        self.screen.blit(self._get_bg(), (0, 0))
        ...
        for i in range(12):
            ...
            pygame.draw.circle(self.screen, col, (cx, cy), int(radius + i * 3), width=2)

        pygame.draw.circle(self.screen, self.core_color, (cx, cy), int(radius), width=3)

        hud = "F11 fullscreen | long=talk | single=screenshot | double=stop | triple=voice"
        txt = self.font.render(hud, True, (110, 165, 175))
        self.screen.blit(txt, (12, 12))

        if self.last_text:
            lines = self._wrap(self.last_text, 64)[:5]
            y = self.h - 22 * (len(lines) + 1)
            for ln in lines:
                if self.mode == "listen":
                    color = (120, 255, 160)
        ...
        pygame.display.flip()
        self.clock.tick(60 if self.mode != "think" else 45)
...
class App:
    ...
    def _capture_user_turn(self, max_total=10.0) -> str:
        # Captures what user said after we detect a real interruption or after AI finishes.
    ...
    def _wait_for_silence_gap(self, gap_sec: float):
        # Guarantees user is done before AI speaks
        silence_start = time.time()
        while time.time() - silence_start < gap_sec and self.ui.running:
            if self.mic_level > VOL_FLOOR:
                silence_start = time.time()
            time.sleep(0.05)
    ...
    def _do_screenshot_analysis(self):
        try:
            ...
            # Do not speak if user is currently speaking
    ...
    def handle_double_press(self):
        self.tts.stop()
        self.ui.set_text("")  # clear subtitles
    ...
    def convo_loop(self):
        def run():
            while self.convo_mode and self.ui.running:
                ...
                # If we stopped AI due to interruption, capture user's turn before replying
                ...
                try:
                    ...
                    # Hard rule: do not start speaking while user is speaking
                    self._wait_for_silence_gap(USER_TURN_END_SILENCE)
                    self.tts.speak_async(reply)

                    # While speaking, if we had a soft interjection pending, capture it AFTER AI finishes
                    while self.tts.is_speaking() and self.convo_mode and self.ui.running:
                        time.sleep(0.03)

                    if self._soft_interjection_pending and self.ui.running:
                        self._soft_interjection_pending = False
                        # capture brief interjection AFTER AI finishes
    ...
    def update_mic_level(self):
        # Read mic queue; always compute RMS smoothing; only update ZCR when needed.
        try:
            while True:
                rms, z = self.mic_q.get_nowait()
                self.mic_level = 0.85 * self.mic_level + 0.15 * min(1.0, rms * 12.0)
        ...
        if user_voice:
            ...
            # If AI is speaking, decide soft vs hard interruption
            if self.tts.is_speaking():
                if speaking_dur >= USER_INTERRUPT_MIN_SEC:
                    # REAL interruption: stop AI, then wait/capture user's full turn
                    ...
                    # Short interjection: allow AI to continue, but mark to capture later
        ...
        # ---- USER SILENCE ----
            ...
            if self._user_speaking_since is not None:
                ...
                # If they spoke a real turn, we consider them "done" once silence gap passes elsewhere.
