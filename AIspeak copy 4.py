...
# =============================================================
# Orb UI (cached background + dt physics + NaN guard + snappy radius)
# =============================================================
...
class OrbUI:
    ...
    # snappy update_radius (your minimal-surgical patch + dynamic stiffness + early kick)
    def update_radius(self, mic_level, dt):
        ...
        raw_err = r_t - self.r
        err = math.copysign(abs(raw_err) ** 0.6, raw_err)

        stretch = min(1.0, abs(self.r - self.base) / (min(self.w, self.h) * 0.45))
        w = 1.8 * math.pi * self.spring_hz * (1.0 - 0.4 * stretch)
        z = self.damping * (6.0 + 8.0 * stretch)

        a = -z * w * self.v + (w * w) * err

        kick = 1.6 if abs(raw_err) < 40 else 1.0
        self.v += (0.00085 * a + kick * err * 0.08) * dt
    ...
    def tick(self, mic_level: float, speaking: bool):
        ...
        # AI animation follows actual speaking (simple, deterministic)
        effective_level = mic_level
        if speaking:
            effective_level = max(mic_level * 0.6, 0.15)

        self.update_radius(effective_level, dt)
        ...
        if not math.isfinite(radius):
            radius = self.base
        radius = max(self.base * 0.6, radius)
