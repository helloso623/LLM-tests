...
# -*- coding: utf-8 -*-
"""
GSM8K evaluator with adaptive multi-sample selection + ONLINE learning (WeightNet) — improved.
Changes:
- Robust final answer extraction from 'Answer:' line (+ fallbacks, sanitization, fractions, commas, trailing punctuation)
- Missing function fixed: is_answer_included (handles str/number; tolerant eps; int vs float)
- Postprocess generations: hard truncate at final 'Answer:' line
- Learner: gradient clipping + weight decay; deterministic init; safe save; optional reset
- Faster closure/logprob: configurable batch size; uses larger batch by default
- Optional torch.compile for small speedup
- Safer dtype/AMP handling; less VRAM churn
- Minor scoring tweaks; clearer CSV; periodic autosave; better k1 split rounding
"""
...
def _to_number_clean(token: str) -> Optional[float]:
    """Convert a cleaned numeric token to float/int, handling commas, trailing dots, signs."""
    ...
    if m:
        ...
        return float(int(m.group(1)))/denom
    try:
...
def extract_number(s: str) -> Optional[float]:
    """Fallback: last numeric in string (with commas); used only if Answer: line missing."""
    m = _NUM_RE.findall(s or "")
    if not m: return None
    return _to_number_clean(m[-1])

def parse_gsm8k_gold(ans: str):
    m = re.search(r"####\s*(-?\d[\d,]*\.?\d*)", ans)
    if not m: return None
    return _to_number_clean(m.group(1))
...
# -------------------- Model & prompts --------------------
def load_model(model_id: str, device: str, dtype: str, compile_ok: bool = True):
    ...
    mdl = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=torch_dtype, device_map=None)
    mdl.to(device).eval()
...
# -------------------- Post-processing: final answer line --------------------
_ANS_LINE = re.compile(r"(?i)^\s*answer\s*:\s*(.+?)\s*$")

def _truncate_after_answer(text: str) -> str:
    """Keep everything up to and including the last 'Answer:' line; drop trailing chatter."""
    lines = (text or "").splitlines()
    last_idx = -1
    for i, ln in enumerate(lines):
        if _ANS_LINE.match(ln):
            last_idx = i
    if last_idx >= 0:
...
def get_final_answer(text: str) -> Optional[float]:
    """Prefer number on final 'Answer:' line; fallback to last numeric anywhere."""
    if not text: return None
    # look for the last Answer: line
    lines = text.splitlines()
    num_on_line = None
    for ln in reversed(lines):
        m = _ANS_LINE.match(ln)
        if not m: continue
        payload = m.group(1)
        # strip units/words like "dollars", ".", etc.
        payload = re.sub(r"[^\d\-\./, ]", " ", payload)
        payload = re.sub(r"\s+", " ", payload).strip()
        # take last numeric token (handles "123 dollars")
        candidates = _NUM_RE.findall(payload)
        if candidates:
            num_on_line = _to_number_clean(candidates[-1])
            break
        # maybe a bare fraction
        if _FRAC_RE.match(payload):
            num_on_line = _to_number_clean(payload)
            break
    if num_on_line is not None:
        return num_on_line
    # fallback
...
def _decode_generated(tok, out_ids, input_lens):
    ...
    for b in range(out_cpu.shape[0]):
        ...
        texts.append(_truncate_after_answer(t))
    return texts

@torch.inference_mode()
def generate_k(tok, mdl, prompt: str, k: int, max_new_tokens: int, temperature: float, top_k: int, top_p: float):
    dev = next(mdl.parameters()).device
    enc = tok([prompt], return_tensors="pt", padding=True, truncation=True, max_length=4096).to(dev)
    input_lens = enc["attention_mask"].sum(dim=1).tolist()
    out = mdl.generate(
        **enc,
        do_sample=True, temperature=temperature, top_p=top_p, top_k=top_k if top_k>0 else 0,
...
# -------------------- Scores: closure + avg logprob (batched) --------------------
@torch.no_grad()
def closure_lp_batch(tok, mdl, prompt: str, answers: List[str], batch_size: int = 4):
    device = next(mdl.parameters()).device
    enc_prefix = tok(prompt.rstrip(), return_tensors="pt", truncation=True, max_length=4096).to(device)
    ...
    for start in range(0, len(answers), batch_size):
        ...
        enc = tok(texts, return_tensors="pt", padding=True, truncation=True, max_length=4096).to(device)
...
def context_vec(tok, mdl, full_prompt: str, ablated_prompt: str, proj_dim: int = 128, seed: int = 1234) -> np.ndarray:
    dev = next(mdl.parameters()).device
    def _last_hidden(p):
        enc = tok(p, return_tensors="pt", truncation=True, max_length=4096).to(dev)
...
class Learner:
    def __init__(self, path="weightnet.pt", in_dim=128, device="cpu", lr=1e-3, wd=1e-4, reset=False, clip=1.0):
        self.path = path
        self.device = device
    ...
    def save(self):
        tmp = self.path + ".tmp"
        torch.save({"model": self.model.state_dict()}, tmp)
...
def _as_float(x) -> Optional[float]:
    ...
    return get_final_answer(str(x))

def equal_num(a, b, eps=1e-3):
    a = _as_float(a); b = _as_float(b)
    if a is None or b is None: return False
    # prefer exact int equality if both near integers
    if abs(a - round(a)) < 1e-9 and abs(b - round(b)) < 1e-9:
        return int(round(a)) == int(round(b))
    return abs(float(a) - float(b)) <= eps

def is_answer_included(text_or_num, gold) -> bool:
    """
    Returns True if the final numeric answer implied by the string (or number) matches gold.
    - If input is str: uses get_final_answer with Answer: preference.
    - If input is numeric: direct compare.
    """
    if isinstance(text_or_num, (int, float)) or text_or_num is None:
        return equal_num(text_or_num, gold)
    # string path
    guess = get_final_answer(text_or_num)
    return equal_num(guess, gold)

# -------------------- Per-item logic --------------------
def evaluate_item(tok, mdl, question: str, gold: float,
                  k: int, max_new_tokens: int, temperature: float, top_k: int, top_p: float,
                  learner: Learner, proj_dim: int, learn_steps: int, lp_batch: int) -> Dict[str, Any]:
    ...
    # Features
    closure, avg_lp = closure_lp_batch(tok, mdl, prompt, texts_all, batch_size=lp_batch)
    ...
    # Teacher (oracle) index (small exploration bonus for phase-2)
    ...
    ok_adapt = is_answer_included(pick_num, gold)

    # Baselines (same pool)
    ok_first = is_answer_included(texts_all[0], gold)
    idx_lp   = int(np.argmax(avg_lp)); ok_lp = is_answer_included(texts_all[idx_lp], gold)
    ...
    ok_sc = is_answer_included(pick_sc, gold)
    ...
    return {
        ...
        "pick_first": get_final_answer(texts_all[0]),
        "pick_lp": get_final_answer(texts_all[idx_lp]),
        "pick_sc": pick_sc,
        "pick_adapt": pick_num,
        "weights": w.tolist(),
        "answer_text": pick_text,
        "candidates": texts_all,
        "features": {
            "closure": closure,
            "avg_logprob": avg_lp,
            "coherence": coh_vals,
...
def main():
    ...
    ap.add_argument("--max_new_tokens", type=int, default=1028)
    ap.add_argument("--temperature", type=float, default=0.7)
    ap.add_argument("--top_p", type=float, default=0.92)
    ap.add_argument("--top_k", type=int, default=0)
    ap.add_argument("--limit", type=int, default=5, help="0 = full GSM8K test (~1319).")
    ...
    ap.add_argument("--learn_wd", type=float, default=1e-4)
    ap.add_argument("--learn_clip", type=float, default=1.0)
    ap.add_argument("--learn_steps", type=int, default=6, help="GD steps per item")
    ...
    ap.add_argument("--lp_batch", type=int, default=4, help="batch size for closure/logprob scoring")
    ...
    tok, mdl = load_model(args.model, args.device, args.dtype, compile_ok=not args.no_compile)
    if args.device == "cuda":
        try:
    ...
    learner = Learner(
        path=args.learn_file,
        in_dim=args.proj_dim,
        device=next(mdl.parameters()).device,
        lr=args.learn_lr,
        wd=args.learn_wd,
        reset=args.reset_memory,
        clip=args.learn_clip
    )
    ...
    for i, (q, gold) in tqdm(list(enumerate(items, 1)), total=N, dynamic_ncols=True, desc="Items"):
        ...
        for k_ in acc: acc[k_] += res[f"ok_{k_}"]
        rows.append([
            q, gold,
